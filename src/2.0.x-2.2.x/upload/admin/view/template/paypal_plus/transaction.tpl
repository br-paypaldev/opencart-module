...
<div id="content">
  ...
  <script type="text/javascript"><!--
    ...
    $('button[name="button-search"]').click(function() {
      $.ajax({
        url: 'index.php?route=paypal_plus/transaction/search&token=<?php echo $token; ?>&paypal_plus_id=' + $('#paypal_plus_id').val(),
        dataType: 'json',
        beforeSend: function() {
      ...
      if (
        ...
        bootbox.confirm({
          ...
          callback: function (result) {
            if (result === true) {
              $.ajax({
                url: 'index.php?route=paypal_plus/transaction/refund&token=<?php echo $token; ?>&paypal_plus_id=' + paypal_plus_id,
                dataType: 'json',
                method: 'post',
                data: formRefund,
                beforeSend: function() {
    ...
    function getDataTransaction(id) {
      $.get('index.php?route=paypal_plus/transaction/dataTransaction&token=<?php echo $token; ?>&paypal_plus_id=' + id, function(data) {
    ...
    $('button[name="button-delete"]').click(function() {
      ...
      bootbox.confirm({
        ...
        callback: function (result) {
          if (result === true) {
            $.ajax({
              url: 'index.php?route=paypal_plus/transaction/delete&token=<?php echo $token; ?>&paypal_plus_id=' + id,
              dataType: 'json',
              beforeSend: function() {
                $(this).prop('disabled', true);
    ...
    $('#button-filter').on('click', function() {
      let filter_url = 'index.php?route=paypal_plus/transaction&token=<?php echo $token; ?>';

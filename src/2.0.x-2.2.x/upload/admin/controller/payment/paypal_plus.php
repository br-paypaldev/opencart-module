...
class ControllerPaymentPayPalPlus extends Controller {
    ...
    const EXTENSIONS = 'extension/payment';
    const PERMISSION = 'paypal_plus';
    ...
    public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            $this->response->redirect($this->url->link(self::EXTENSIONS, 'token=' . $this->session->data['token'], true));
        }
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link(self::EXTENSIONS, 'token=' . $this->session->data['token'], true)
        );
        ...
        $data['cancel'] = $this->url->link(self::EXTENSIONS, 'token=' . $this->session->data['token'], true);
        ...
        if (version_compare(VERSION, '2.1') < 0) {
            ...
            $data['customer_groups_data'] = $this->model_sale_customer_group->getCustomerGroups();
        } else {
        ...
        $this->response->setOutput($this->load->view(self::EXTENSION . '.tpl', $data));
    }

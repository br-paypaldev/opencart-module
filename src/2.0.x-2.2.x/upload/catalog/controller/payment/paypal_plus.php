...
class ControllerPaymentPayPalPlus extends Controller {
    ...
    public function index() {
        ...
        if (version_compare(VERSION, '2.2') < 0) {
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/'. self::EXTENSION . '.tpl')) {
                return $this->load->view($this->config->get('config_template') . '/template/'. self::EXTENSION . '.tpl', $data);
            } else {
                return $this->load->view('default/template/'. self::EXTENSION . '.tpl', $data);
            }
        } else {
